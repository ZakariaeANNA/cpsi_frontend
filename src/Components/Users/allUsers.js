            ...
            >
                ...
                <DialogContent dividers>
                    <div class="container" id="addUser">
                        <div class='section-admin shadow-2xl'>
                                            ...
                                            <div class="is-grid">
                                                <div class="flex flex-col justify-between">
                                                    <div class="form-group">
                                                        ...
                                                        value={params.nomEtablissement} required/>
                                                    </div>
                                                    <div class="form-group mt-2">
                                                        <label for="codeGRESA" class="form-label">Code GRESA</label>
                                                        <input id="codeGRESA" maxlength="30" type="text" name="codeGRESA" value={params.codeGRESA} required/>
                                                    </div>
                                                    <div class="form-group mt-2">
                                                        <label for="delegation" class="form-label">Délégation</label>
                                                        <input id="delegation" maxlength="30" type="text" name="delegation" value="Tétouan" required/>
                                                    </div>
                                                    <div class="form-group mt-2">
                                                    ...
                                                    <label for="name" class="form-label">Nom de directeur</label>
                                                        <input id="name" type="text" maxlength="30" value={params.nomDirecteur} name="name"  required/>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
function AddUser(){
    ...
    return(
        <div className="container flex justify-end" id="addUser">
            <Button onClick={handleClickOpen} className="h-4 flex">
                <div className="text-decoration-none btn btn-primary action flex items-center p-2">
                    <svg xmlns="http://www.w3.org/2000/svg" height="20" className="mr-0.5 bi bi-plus-circle" fill="currentColor" viewBox="0 0 16 16">
                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                        <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
                    </svg> &nbsp;<span className="text-xs w-full"> Ajouter un Utilisateur</span>
                </div>
            </Button>
            <BootstrapDialog
                onClose={handleClose}
...
function AllUsers(){
    ...
    const OriginalRows = [
        {id: 1, codeGRESA: '05380X',nomDirecteur: 'ecole THAMI EL OUAZZANI', numeroTelephone: 'null', nomEtablissement: 'ecole THAMI EL OUAZZANI',typeEtablissement: 'unite'},
        {id: 2, codeGRESA: '05380X',nomDirecteur: 'ecole THAMI EL OUAZZANI', numeroTelephone: 'null', nomEtablissement: 'ecole THAMI EL OUAZZANI',typeEtablissement: 'g-scolaire'},
        {id: 3, codeGRESA: '05380X',nomDirecteur: 'ecole THAMI EL OUAZZANI', numeroTelephone: 'null', nomEtablissement: 'ecole THAMI EL OUAZZANI',typeEtablissement: 'lycee'},
        {id: 4, codeGRESA: '05380X',nomDirecteur: 'ecole THAMI EL OUAZZANI', numeroTelephone: 'null', nomEtablissement: 'ecole THAMI EL OUAZZANI',typeEtablissement: 'primaires'},
        {id: 5, codeGRESA: '05380X',nomDirecteur: 'ecole THAMI EL OUAZZANI', numeroTelephone: 'null', nomEtablissement: 'ecole THAMI EL OUAZZANI',typeEtablissement: 'colleges'},
        {id: 6, codeGRESA: '05380X',nomDirecteur: 'ecole THAMI EL OUAZZANI', numeroTelephone: 'null', nomEtablissement: 'ecole THAMI EL OUAZZANI',typeEtablissement: 'primaires'},
        {id: 7, codeGRESA: '05380X',nomDirecteur: 'ecole THAMI EL OUAZZANI', numeroTelephone: 'null', nomEtablissement: 'ecole THAMI EL OUAZZANI',typeEtablissement: 'lycee'},
        {id: 8, codeGRESA: '05380X',nomDirecteur: 'ecole THAMI EL OUAZZANI', numeroTelephone: 'null', nomEtablissement: 'ecole THAMI EL OUAZZANI',typeEtablissement: 'lycee'},
        {id: 9, codeGRESA: '05380X',nomDirecteur: 'ecole THAMI EL OUAZZANI', numeroTelephone: 'null', nomEtablissement: 'ecole THAMI EL OUAZZANI',typeEtablissement: 'colleges'},
     
    ]
    const [rows,setRows] = useState(OriginalRows)
    const SchoolNameFilter = (text)=>{
        let filtered = OriginalRows.filter((row)=>(
            row.nomEtablissement.toLowerCase().includes(text.toLowerCase())
        ))
        if(filtered){
            setRows(filtered)
        }else{
    ...
    const SchoolTypeFilter = (type)=>
    {
        if(type!=='all'){
            let filtered = OriginalRows.filter((row)=>(
            row.typeEtablissement===type
            ))
            if(filtered){
                setRows(filtered)
            }else{
                    ...
                    <div className='outer-block-body'>
                        <div className="w-full flex justify-center items-center mb-3">
                            <form className="h-10 w-1/2 mr-2 flex justify-between" action="#result">
                                <input className="w-full mr-0" name="search" onChange={(text)=>SchoolNameFilter(text.target.value)} type="text" placeholder="Commencez à taper pour rechercher des établissements ..." aria-label="Search" required/>
                            </form>
                            <form className="h-10 w-1/2 flex justify-between" action="#result">
                                <select className="w-full form-select mr-0" onChange={(text)=>SchoolTypeFilter(text.target.value)} name="typeEtablissement">
